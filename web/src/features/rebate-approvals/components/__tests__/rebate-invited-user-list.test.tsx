...
describe('RebateInvitedUserList', () => {
  test('shows each invitee with registration, login, fallback, and deleted status', async () => {
    ...
    expect(markup).toContain('Invited User Details')
    expect(markup).toContain('Active User')
    expect(markup).toContain('deleted-user')
    expect(markup).toContain('Created At')
    expect(markup).toContain('Last Login')
    expect(markup).toContain('>-</span>')
    expect((markup.match(/>Deleted</g) ?? []).length).toBe(1)
  })

  test('shows an empty state when the applicant has no invitees', async () => {
    expect(await renderInvitedUserList([])).toContain('No invited users found.')
  })
})
